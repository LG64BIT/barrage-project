...
pub fn router(conf: &mut ServiceConfig) {
    ...
    conf.service(web::resource("/products").route(web::get().to(products::single::handle)));
    conf.service(
        web::resource("/products/{product_id}").route(web::get().to(products::all::handle)),
    );
    conf.service(web::resource("/cart/{product_id}").route(web::post().to(cart::add::handle)));
    conf.service(web::resource("/cart").route(web::get().to(cart::get::handle)));
    conf.service(web::resource("/cart").route(web::delete().to(cart::empty::handle)));
}
